...
function Landmark({ position, color, label, size, onClick, onHover }) {
  return (
    <group position={position} onPointerOver={onHover} onClick={onClick}>
      <mesh>
        <boxGeometry args={[size || 0.5, size || 0.5, size || 0.5]} />
        <meshStandardMaterial color={color || 'gray'} emissive={color} emissiveIntensity={0.5} />
      </mesh>
      {label && (
