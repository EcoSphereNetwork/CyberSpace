...
export class DigitalLayer extends Layer {
  ...
  private async createConnection(config: DigitalConnection): Promise<Object3D> {
    ...
    if (!fromNode || !toNode) {
      throw new Error(
        `Invalid connection nodes: ${config.from} -> ${config.to}`
      );
    }
    ...
    const material = (
      this.resources.materials.get('connection') as LineBasicMaterial
    ).clone();
    if (config.color) {
  ...
  private createParticle(): DigitalParticle {
    return {
      ...
      velocity: new Vector3(
        ...
        .normalize()
        .multiplyScalar(this.digitalConfig.particles?.speed ?? 1),
  ...
  public setParticleProperties(properties: {
    ...
    if (properties.color !== undefined) {
      (this.resources.materials.get('particle') as PointsMaterial).color.setHex(
        properties.color
      );
    }
    if (properties.size !== undefined) {
      (this.resources.materials.get('particle') as PointsMaterial).size =
        properties.size;
    }
