...
const roles = {
   ...
   */
  export function lockUser(username) {
    const user = users.get(username);
    if (!user) {
   ...
   */
  export function handleFailedLogin(username) {
   ...
   */
  export function logAudit(message, level = 'info') {
   ...
   */
  export function rateLimit(fn, limit, interval) {
    const calls = [];
